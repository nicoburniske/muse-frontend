...
export const useAddTracksToPlaylistMutation = (options?: UseMutationOptions<unknown, unknown, AddToPlaylistParams, unknown>) => {
    const client = useSpotifyClient()

    return useMutation(['AddTracksToPlaylist'],
        ({ playlistId, trackIds, position }: AddToPlaylistParams) => client.playlists.addItemsToPlaylist(playlistId, trackIds.map(t => toUri('Track', t)), { position }),
        options)
}

type ReorderParams = { playlistId: string, rangeStart: number, insertBefore: number, rangeLength?: number }
export const useReorderPlaylistTracksMutation = (options?: UseMutationOptions<unknown, unknown, ReorderParams, unknown>) => {
    const client = useSpotifyClient()

    return useMutation(['MovePlaylistTracks'],
...
export const useRemoveTracksFromPlaylistMutation = (options?: UseMutationOptions<unknown, unknown, { playlistId: string, trackIds: string[] }, unknown>) => {
    const client = useSpotifyClient()

    return useMutation(['RemoveTracksFromPlaylist'],
        ({ playlistId, trackIds }: { playlistId: string, trackIds: string[] }) => client.playlists.removePlaylistItems(playlistId, trackIds.map(t => toUri('Track', t))),
        options
    )
}
...
export const useAddTrackToQueue = (options?: UseMutationOptions<unknown, unknown, string, unknown>) => {
    ...
    return useMutation(['AddTracksToQueue'],

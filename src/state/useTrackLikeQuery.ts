...
const createDataLoader = (spotifyClient: SpotifyWebApi) => (
    new DataLoader<string, boolean>(async (trackIds: readonly string[]) =>
        spotifyClient.library.areTracksSaved(trackIds as string[])
...
export const useTrackLikeQuery = (trackId: string, options?: UseQueryOptions<boolean, unknown, boolean, string[]>) => {
    ...
    const updateLike = useCallback((like: boolean) => {
        invalidate()
